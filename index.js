...
async function addEmployees() {
  const str = await inquirer.prompt([
    ...
    },
    {
      type: "message",
      name: "role",
      message: "What is this this employees role id? (1-14)"
    },
  ]);
...
async function addRole() {
  ...
  switch (choice) {
    case "maintenance":
      db.query(`INSERT INTO roles(title, salary, department_id) VALUES("Maintenance", "30000", "8")`, async function (err, result) {
        if (err) throw err;
        if (result) {
          console.table(result);
          await loadPrompts();
      ...
      break;
    case "forklift":
      db.query(`INSERT INTO roles(title, salary, department_id) VALUES("Forklift Operator", "25000", "8")`, async function (err, result) {
        if (err) throw err;
        if (result) {
          console.table(result);
          await loadPrompts();
      ...
      break;
    case "janitor":
      db.query(`INSERT INTO roles(title, salary, department_id) VALUES("Janitor", "20000", "8")`, async function (err, result) {
        if (err) throw err;
        if (result) {
          console.table(result);
          await loadPrompts();
